...
Move the `DATE_create_permission_table.php` generated by Spatie Permissions from the `migrations` folder to `migrations/tenant` folder.
...
`App\User.php` - notice the `HasRoles`, `UsesSystemConnection` traits we added.
```php
<?php
